...
4. Merge only the validated head commit.
5. Watch **Deploy DivyaNexus React App** until both build and deploy jobs succeed.
6. Watch **Verify DivyaNexus Production** until release, commit, assets, health, and direct-route checks succeed.
7. Open `/status` and compare the HTML marker, React marker, and declared release.
8. Verify `/release.json` contains the merged commit SHA.
9. Record the merge commit, deployment run, verification run, and completion time.
...
- Direct routes load without the loading-screen defect or Not Found page.
...
- Direct-route failure: inspect the generated `404.html` or SPA fallback behaviour.
