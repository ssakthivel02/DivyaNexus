...
function webpDimensions(buffer) {
  if (buffer.length < 30 || buffer.toString("ascii", 0, 4) !== "RIFF" || buffer.toString("ascii", 8, 12) !== "WEBP") {
    throw new Error("Asset is not a RIFF WebP file");
  }
...
if (existsSync(assetPath)) {
  ...
  try {
    dimensions = webpDimensions(asset);
  } catch (error) {
...
if (dimensions) {
  ...
  if (dimensions.width < 1200 || dimensions.height < 675) {
    failures.push(`Artwork resolution is below 1200×675 (${dimensions.width}×${dimensions.height})`);
  }
...
if (!component.includes('loading="lazy"')) failures.push("Owner artwork must remain lazy-loaded below the fold");
...
if (!serviceWorker.includes("divyanexus-stage-b-wave7")) failures.push("Service-worker cache was not versioned for Wave 7");
...
console.log(`Owner-artwork validation passed: ${dimensions.width}×${dimensions.height}, ${bytes} bytes, repository-owned WebP, uncropped and offline-ready.`);
